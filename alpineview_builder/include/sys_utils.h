...
#define ASSUME_ALIGNED(var, size)                                              \
	do {                                                                       \
		var = VOIDSTARCAST(var) __builtin_assume_aligned(var, size);           \
	} while (0)
#else
#define LIKELY(x) (x)
#define UNLIKELY(x) (x)
...
#define MALLOC_NUM(ptr, num)                                                   \
	do {                                                                       \
		ptr = VOIDSTARCAST(ptr) malloc((num) * sizeof(*ptr));                  \
		if (UNLIKELY(num != 0 && ptr == NULL))                                 \
			abort();                                                           \
	} while (0)

#define MALLOC_SIZ(ptr, size)                                                  \
	do {                                                                       \
		ptr = VOIDSTARCAST(ptr) malloc((size));                                \
		if (UNLIKELY(size != 0 && ptr == NULL))                                \
			abort();                                                           \
	} while (0)

#define CALLOC_NUM(ptr, num)                                                   \
	do {                                                                       \
