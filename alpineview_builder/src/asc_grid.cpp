...
static int read_key_value(FILE *f, const char *want, double &out)
{
	char key[64];
	double value;
...
int asc_read_header(const char *path, AscHeader &hdr)
{
	...
	int ret = read_key_value(f, "ncols", ncols) ||
		  read_key_value(f, "nrows", nrows) ||
		  read_key_value(f, "xllcorner", xll) ||
		  read_key_value(f, "yllcorner", yll) ||
		  read_key_value(f, "cellsize", cellsize) ||
		  read_key_value(f, "nodata_value", nodata);
	fclose(f);
	if (ret)
		return (-1);
...
static void index_dir(const char *dir, std::vector<AscTile> &tiles)
{
	DIR *d = opendir(dir);
	if (!d)
		return;
...
int asc_index(const char *dir, std::vector<AscTile> &tiles)
{
	...
	std::sort(tiles.begin(), tiles.end(),
		  [](const AscTile &a, const AscTile &b) {
			  return a.path < b.path;
		  });
	return (int)tiles.size();
}

static void tile_extent(const AscHeader &h, double &x0, double &y0, double &x1,
			double &y1)
{
	x0 = h.xll;
	y0 = h.yll;
...
static int load_tile(AscTile &tile)
{
			 ...
			 double x1, double y1)
{
	...
	for (AscTile &tile : tiles) {
		...
		tile_extent(tile.hdr, tx0, ty0, tx1, ty1);
...
static float cell_value(const AscTile &t, int col, int row)
{
...
float asc_sample(const std::vector<AscTile> &tiles, double x, double y)
{
	for (const AscTile &t : tiles) {
		if (t.z.empty())
			continue;
		double tx0, ty0, tx1, ty1;
		tile_extent(t.hdr, tx0, ty0, tx1, ty1);
		...
		float v00 = cell_value(t, col, row);
		float v10 = cell_value(t, col + 1, row);
		float v01 = cell_value(t, col, row + 1);
		float v11 = cell_value(t, col + 1, row + 1);
