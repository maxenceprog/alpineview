...
double calculateError(int idV1, int idV2, vec3f &pResult);
bool flipped(vec3f p, int i0, int i1, Vertex &v0, Vertex &v1,
			 std::vector<int> &deleted);
void updateUvs(int i0, const Vertex &v, const vec3f &p,
			   std::vector<int> &deleted);
void updateTriangles(int i0, Vertex &v, std::vector<int> &deleted,
					 int &deletedTriangles);
void updateMesh(int iteration);
void dropDeletedTriangles();
//
...
void simplifyMesh(int targetCount, double agressiveness = 7,
	...
	for (int iteration = 0; iteration < 100; iteration++) {
		...
		//
		double threshold =
			0.000000001 * pow(double(iteration + 3), agressiveness);

		// target number of triangles reached ? Then break
		if ((verbose) && (iteration % 5 == 0)) {
			printf("iteration %d - triangles %d threshold %g\n", iteration,
				   triangleCount - deletedTriangles, threshold);
		}
		...
		loopi(0, triangles.size()) {
			...
			loopj(0, 3) if (t.err[j] < threshold) {
				...
				if (tcount <= v0.tcount) {
					// save ram
					if (tcount)
						memcpy(&refs[v0.tstart], &refs[tstart],
							   tcount * sizeof(Ref));
				} else
					// append
					v0.tstart = tstart;
...
void updateMesh(int iteration) {
	...
	if (iteration == 0) {
		...
		loopi(0, vertices.size()) {
			...
			loopj(0, vcount.size()) if (vcount[j] == 1) vertices[vids[j]]
				.border = 1;
		}
		// initialize errors
...
double calculateError(int idV1, int idV2, vec3f &pResult) {
	...
	SymetricMatrix q = vertices[idV1].q + vertices[idV2].q;
	...
	if (det != 0 && !border) {
		...
		pResult.x = -1 / det *
					(q.det(1, 2, 3, 4, 5, 6, 5, 7, 8)); // vx = A41/det(q_delta)
		pResult.y = 1 / det *
					(q.det(0, 2, 3, 1, 5, 6, 2, 7, 8)); // vy = A42/det(q_delta)
		pResult.z = -1 / det *
					(q.det(0, 1, 3, 1, 4, 6, 2, 5, 8)); // vz = A43/det(q_delta)

		error = vertexError(q, pResult.x, pResult.y, pResult.z);
	} else {
		// det = 0 -> try to find best result
		vec3f p1 = vertices[idV1].p;
		vec3f p2 = vertices[idV2].p;
		vec3f p3 = (p1 + p2) / 2.0;
...
// Option : Load OBJ
void loadObj(const char *filename, bool processUv = false) {
	vertices.clear();
	triangles.clear();
	...
	while (fgets(line, 1000, fn) != NULL) {
		...
		if (strncmp(line, "usemtl", 6) == 0) {
			...
			if (materialMap.find(usemtl) == materialMap.end()) {
				materialMap[usemtl] = materials.size();
				materials.push_back(usemtl);
			}
			material = materialMap[usemtl];
		}
		...
		if (line[0] == 'v' && line[1] == 't') {
			if (line[2] == ' ') {
				if (sscanf(line, "vt %lf %lf", &uv.x, &uv.y) == 2) {
					uv.z = 0;
					uvs.push_back(uv);
				} else if (sscanf(line, "vt %lf %lf %lf", &uv.x, &uv.y,
								  &uv.z) == 3) {
					uvs.push_back(uv);
				}
			}
			...
			if (line[1] == ' ')
				if (sscanf(line, "v %lf %lf %lf", &v.p.x, &v.p.y, &v.p.z) ==
					3) {
		...
		if (line[0] == 'f') {
			...
			if (sscanf(line, "f %d %d %d", &integers[0], &integers[1],
					   &integers[2]) == 3) {
				triOk = true;
			} else if (sscanf(line, "f %d// %d// %d//", &integers[0],
							  &integers[1], &integers[2]) == 3) {
				triOk = true;
			} else if (sscanf(line, "f %d//%d %d//%d %d//%d", &integers[0],
							  &integers[3], &integers[1], &integers[4],
				...
				if (sscanf(line, "f %d/%d %d/%d %d/%d", &integers[0],
						   &integers[6], &integers[1], &integers[7],
			...
			if (triOk) {
				t.v[0] = integers[0] - 1 - vertexCnt;
				t.v[1] = integers[1] - 1 - vertexCnt;
				t.v[2] = integers[2] - 1 - vertexCnt;
				t.attr = 0;

				if (processUv && hasUv) {
					std::vector<int> indices;
					indices.push_back(integers[6] - 1 - vertexCnt);
					indices.push_back(integers[7] - 1 - vertexCnt);
					indices.push_back(integers[8] - 1 - vertexCnt);
...
void writeObj(const char *filename) {
	...
	int curMaterial = -1;
	bool hasUv =
		(!triangles.empty() && (triangles[0].attr & TEXCOORD) == TEXCOORD);
	...
	int uv = 1;
	loopi(0, triangles.size()) if (!triangles[i].deleted) {
		if (triangles[i].material != curMaterial) {
			curMaterial = triangles[i].material;
			fprintf(file, "usemtl %s\n",
					materials[triangles[i].material].c_str());
		}
		if (hasUv) {
			fprintf(file, "f %d/%d %d/%d %d/%d\n", triangles[i].v[0] + 1, uv,
					triangles[i].v[1] + 1, uv + 1, triangles[i].v[2] + 1,
					uv + 2);
			uv += 3;
		} else {
...
void loadMz3(const char *filename) {
	vertices.clear();
	triangles.clear();
	int material = -1;
	std::map<std::string, int> materialMap;
	...
	struct Mz3hdr h;
	size_t bytesRead = fread(&h, sizeof(struct Mz3hdr), 1, fp);
	if (bytesRead <= 0) {
	...
	bytesRead = fread(tris, tribytes, 1, fp);
	if (bytesRead <= 0) {
	...
	bytesRead = fread(verts32, vertbytes32, 1, fp);
	if (bytesRead <= 0) {
...
void writeMz3(const char *filename) {
